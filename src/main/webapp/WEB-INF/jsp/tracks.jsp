...
<%@ page import="com.slimechan.raceway_system.model.TrackModel" %>
...
<div class="add-data" id="add-panel" hidden>
    ...
    <h3 class="title">Добавление трека</h3>
    <table>
        <tr name="idinfo" hidden><td>Id</td><td name="id"></td></tr>
        <tr><td>Назввание</td><td><input type="text" name="name"></td></tr>
        <tr><td>Описание</td><td><textarea name="desc"></textarea></td></tr>
        <tr><td>Конфигурация</td><td><select name="config">
            <%
                for (TrackModel.TrackConfig cfg : TrackModel.TrackConfig.values()) {
                    out.println("<option value=\""+cfg.toString().toLowerCase()+"\">"+cfg+"</option>");
                }%>
        </select></td></tr>
        <tr><td><input type="button" value="Send" onclick="tryAdd()"></td></tr>
    </table>
</div>
<div class="logo-head">
...
<div class="main-wrapper">
    <div class="left-menu">
        ...
        <table>
            ...
            <tr>
                <td class="active" onclick="menuClick(this)">Tracks <img class="icon" src="css/img/icon/track.png" oncontextmenu="return false" ondragstart="return false"/></td>
            </tr>
            <%
    ...
    <div class="main-block">
        <div class="block-content" id="news">
            <% List<List<TrackModel>> news = (List<List<TrackModel>>)request.getAttribute("tracks");
                for(int y =0; y<news.size();y++){
                    out.println("<div class='news-row'>");
                    int count = 0;
                    for(TrackModel nm : news.get(y))
                    {
                        if(nm==null){ System.out.println("null "+count);continue;}
                        out.println("<div class=\"news-block\" style='background:"+nm.getConfigColor()+
                                "'>" +
                                "<button class=\"remove\" onclick=\"tryRemove(this)\"></button>" +
                                "<div class=\"news-info\">" +
                                nm.getName().substring(0, nm.getName().length()>40?40:nm.getName().length()) +
                                "<p hidden>Конфигурация: " +
                                nm.getConfig() +
                                "</p>" +
                    ...
                    if(count==6)out.println("</div>");
                    else{
                        out.println("<div class=\"news-block add\" onclick=\"add()\">+</div>");
                        out.println("</div>");
                    }
                }
                if(news.size()==0 || news.get(0).size()*news.size()%6==0){
                    out.println("<div class='news-row'>");
                    out.println("<div class=\"news-block add\" onclick=\"add()\">+</div>");
